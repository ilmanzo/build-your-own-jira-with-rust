...
impl TicketStore {
    ...
    pub fn update(&mut self, id: &TicketId, patch: TicketPatch) -> Option<&Ticket> {
        match self.data.get_mut(id) {
            None => None,
            Some(ticket) => {
                if let Some(patched_title) = patch.title {
                    ticket.title = patched_title;
                }
                if let Some(patched_description) = patch.description {
                    ticket.description = patched_description;
                }
                if let Some(patched_status) = patch.status {
                    ticket.status = patched_status;
                }
